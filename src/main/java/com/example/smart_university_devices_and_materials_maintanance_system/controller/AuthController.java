...
public class AuthController {
    ...
    public String processRegister(
            ...
            @RequestParam(required = false) String newUniversityName,
            @RequestParam(required = false) String collegeName,
            @RequestParam(required = false) String newCollegeName,
            RedirectAttributes redirectAttrs,
            Model model) {
        ...
        try {
            ...
            if (newUniversityName != null && !newUniversityName.trim().isEmpty()) {
                university = universityRepository.findByName(newUniversityName)
                        .orElseGet(() -> universityRepository.save(University.builder()
                                .name(newUniversityName)
                                .abbreviation(newUniversityName.substring(0, Math.min(3, newUniversityName.length())).toUpperCase())
            ...
            String finalCollegeName = (newCollegeName != null && !newCollegeName.trim().isEmpty()) ? newCollegeName : collegeName;
            
            final University finalUniversity = university;
            if (finalCollegeName != null && !finalCollegeName.trim().isEmpty() && finalUniversity != null) {
                String name = finalCollegeName.trim();
                college = collegeRepository.findByNameAndUniversityId(name, finalUniversity.getId())
                        .orElseGet(() -> collegeRepository.save(College.builder()
                                .name(name)
                                .abbreviation(name.substring(0, Math.min(3, name.length())).toUpperCase())
