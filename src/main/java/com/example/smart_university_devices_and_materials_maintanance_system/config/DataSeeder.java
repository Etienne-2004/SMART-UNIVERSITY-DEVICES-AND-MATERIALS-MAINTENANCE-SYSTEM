...
public class DataSeeder implements CommandLineRunner {
    ...
    private void seedUniversities() {
        ...
        if (!universityRepository.existsByName("University of Rwanda")) {
        ...
        for (Map.Entry<String, String> e : polytechnics.entrySet()) {
            if (!universityRepository.existsByName(e.getKey())) {
        ...
        for (String[] ns : nursingSchools) {
            if (!universityRepository.existsByName(ns[0])) {
                universityRepository.save(University.builder()
    ...
    private void seedMoreUsers() {
        University ur = universityRepository.findAll().stream()
                .filter(u -> "University of Rwanda".equals(u.getName()))
                .findFirst()
                .orElse(null);
        if (ur == null) return;
        
        List<College> colleges = collegeRepository.findByUniversityId(ur.getId());
        ...
        for (int i = 0; i < names.length; i++) {
            ...
            if (!userRepository.existsByUsername(username)) {
                userRepository.save(User.builder()
                        ...
                        .password(passwordEncoder.encode("Password@2026"))
                        .fullName(names[i])
                        .role(roles[i])
                        .mfaEnabled(false)
                ...
                u.setPassword(passwordEncoder.encode("Password@2026"));
    ...
    private void seedDevices() {
        ...
        University ur = universityRepository.findAll().stream()
                .filter(u -> "Integrated Polytechnic Regional Center Karongi".equals(u.getName()) || "University of Rwanda".equals(u.getName()))
                .findFirst()
                .orElse(null);
